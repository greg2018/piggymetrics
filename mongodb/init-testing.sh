...
create_user="if (!db.getUser('user')) { db.createUser({ user: 'user', pwd: '$MONGODB_PASSWORD', roles: [ {role:'readWrite', db:'piggymetrics'}, {role:'userAdminAnyDatabase', db:'piggymetrics'},{role:'readWriteAnyDatabase', db:'piggymetrics'}  ]}) }"
